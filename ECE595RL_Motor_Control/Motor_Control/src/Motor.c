...
void Motor_Forward(uint16_t left_duty_cycle, uint16_t right_duty_cycle)
{
    ...
    // by clearing Bits 4 and 5 of the OUT register for P5 - PH(Phase) -  DIRR and DIRL
    P5->OUT &= ~0x30;

    // Update the duty cycle for both motors
    // Here's where we update the EN - PWMR and PWML - pins P2.6 and P2.7
    Timer_A0_Update_Duty_Cycle_1(right_duty_cycle);
    Timer_A0_Update_Duty_Cycle_2(left_duty_cycle);

    // Enable the motors by setting Bits 6 and 7 of the OUT register for P3 - nSLEEP
    P3->OUT |= 0xC0;
}
